...
public class PropertyUtil implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    private void loadSetting() {
        try {
            ...
            String datasourceType = getString(PropertiesConstant.SPRING_DATASOURCE_PLATFORM, "");
            setUseExternalDB(PropertiesConstant.MYSQL
                    .equalsIgnoreCase(datasourceType) || PropertiesConstant.POSTGRES
                    .equalsIgnoreCase(datasourceType));
            ...
            //这里需要判断是否使用外部存储库，主要就是通过上面的setUseExternalDB方法判断的
            if (isUseExternalDB()) {
