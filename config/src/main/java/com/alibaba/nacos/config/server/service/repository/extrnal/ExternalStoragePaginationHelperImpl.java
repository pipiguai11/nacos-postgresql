...
class ExternalStoragePaginationHelperImpl<E> implements PaginationHelper {
    ...
    public Page<E> fetchPage(final String sqlCountRows, final String sqlFetchRows, final Object[] args,
        ...
        selectSql = handlerLimit(selectSql,startRow,pageSize);
    ...
    public Page<E> fetchPageLimit(final String sqlCountRows, final String sqlFetchRows, final Object[] args,
        ...
        selectSql = handlerLimit(selectSql,0,0);
    ...
    public void updateLimit(final String sql, final Object[] args) {
        ...
        sqlUpdate = handlerLimit(sqlUpdate,0,0);
    ...
    /**
     * 处理limit格式，在postgres中不支持limit x,y的写法，但是支持limit x offset y 的写法
     * @param sql
     * @param start
     * @param size
     * @return
     */
    private String handlerLimit(String sql, int start, int size){
        String result = sql;
        //如果遇到limit问题，需要将limit a,b 替换成limit a offset b
        if (PropertiesConstant.POSTGRES.equalsIgnoreCase(EnvUtil.getProperty(PropertiesConstant.SPRING_DATASOURCE_PLATFORM))){
            if (start == 0 && size == 0){
                result = result.replaceAll("limit \\?,\\?", "limit ? offset ?");
            }else {
                //将数字绑定上去
                result = result.substring(0,result.indexOf("limit")) + " limit " + size + " offset " + start;
            }
